...
var options = defaultOptions, schema = defaultSchema;

var setupFormBuilder = (new function () {
    ...
    var register = function (config) {
        ...
        var initManageForm = function () {
            if ($('.manage').alpaca("get"))
                $('.manage').alpaca("destroy");
            $('.manage').alpaca({
                "schema": {
                    "type": "object",
                    "required": false,
                        ...
                        form.registerSubmitHandler(function (e, form) {
                            ...
                            if (form.isFormValid())
                                if (configuration && configuration.save) {
                                    ...
                                    var data = { "form": formData, "common": common };

                                    configuration.save.call(null, data);
                                }
                            e.stopPropagation();
                            return false;
                        });
        ...
        $(".clear-results").off().click(function () {
            //add notifier
            editor1.setValue(JSON.stringify(defaultSchema));
            editor1.clearSelection();
            editor2.setValue(JSON.stringify(defaultOptions));
            editor2.clearSelection();
            editor3.setValue(JSON.stringify({}));
            editor3.clearSelection();
            refreshView();
            refreshPreview();
            refreshDesigner();

            //$('.manage').alpaca("clearControl");
            initManageForm();
        });
        ...
        //                            }
        //                            var data = { "form": form, "common": common };
